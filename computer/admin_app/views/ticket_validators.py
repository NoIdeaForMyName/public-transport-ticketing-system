...
class TicketValidatorsView(AbstractFrame):
    ...
    def on_appear(self):
        ...
        # Add data rows
        for i, ticket_validator_one in enumerate(ticket_validators.get_all_ticket_validators()):  # start=2 because of header and separator
            draw_at_row = i * 2 + 2  # Account for separators
            # Registration number (centered)
            ttk.Label(
                self.scrollable_frame,
                text=ticket_validator_one.ip_address,
                font=APP_FONT,
                anchor="center"
            ...
            ttk.Button(
                ...
                text="Usuń",
                style="SmallButton.TButton",
                command=lambda: self.delete_ticket_validator(ticket_validator_one))
